...
for j=1:length(W)
    Amatrix=[Xu(j)      Xw(j)        0      -g*cos(theta0)
        ...
        Mu+((Mw_dot*Zu(j))./(1-Zw_dot(j)))        Mw+((Mw_dot*Zw(j))./(1-Zw_dot(j)))        Mq+((U0+Zq(j))*Mw_dot)./(1-Zw_dot(j))      -g*sin(theta0)*Mw_dot./(1-Zw_dot(j))
        0        0       1       0];
    EigenA4=eig(Amatrix);
    Fourth_O_eig_matrix(:,j)=EigenA4;
    %% Short period & Damping ratio and frequency
    Ashort=[Zw(j)./(1-Zw_dot(j)) (U0+Zq(j))./(1-Zw_dot(j))
        Mw+(Mw_dot*Zw(j))./(1-Zw_dot(j)) Mq+((U0+Zq(j))*Mw_dot)./(1-Zw_dot(j))];
    EigShort=eig(Ashort);
