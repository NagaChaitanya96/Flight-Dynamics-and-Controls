...
global Xu  Zu  U0 theta0 Mu Zw Xw Mw Zw_dot Mw_dot Zq Mq CD_0 CL_0 j


% Approach: Variation in CG occurs when there is STATIC MARGIN, => dcm/dcl=-(SM).
% we know that Cmalpha=dcm/dcl*Clalpha; since SM=Xnp-Xcg; we find initial
% dcm/dcl =CMalpha/Clalpha; and from that due to variation in it by+-10 %
% we can vary Cmalpha 
...
CLdeltaE=0.26;
Cm0=0.05;
% Taking Initial CMalpha=Cmalpha0
Cmalp0=-.59;
CLalp_tail=0;
efficiency=0.9;
VH=0.7;
dEpsdAlp=2*CLalp/(pi*AR);
dcmdcl=Cmalp0/CLalp;
% Finding the neutral point, since Clalpha tail is not know ,we assume it
% to be very small and consider 0
Xnp=0.25+ (1-dEpsdAlp)*efficiency*VH*CLalp_tail                    %taking Neutral point at Aerodynamic center
Xcg_var=(Xnp+dcmdcl).*[0.9 1 1.3];      % neutral p @ 2.29
dcmdcl_vec=Xcg_var-0.25;
Cmalp=dcmdcl_vec.*CLalp;
Cmq=-9.3;
Cmalp_dot=-4.3;
CmdeltaE=-1.008;
...
CL=(W/S)./(qinf);
CD=(Thrust/S)./(qinf);
Cm=0; % For aircraft at trim
%% Finding alpha, deltaE, from the CL,Cm, and from Longitudinal model equations
for i=1:length(Xcg_var)
    A=[CLalp CLdeltaE;Cmalp(i) CmdeltaE];
    dcl=CL -CL0  ;   % delta cl
    B=[dcl;Cm-Cm0];
    x=A\B;
    X(:,i)=x;
...
for j=1:length(Cmalp)
    ...
    title('Plotting all Eigen values locus for variation in location of CG')
    xlabel( '{\zeta\omega_n}')
    ylabel('{\omega_d}')
end

legend('0.9*CG','CG','1.1*CG')
