...
H=2000;
Vinf=60 ;
% V1=[35 40 42];       % Variation of speed +-25m/s
V1=25:120;
% V1=25:120
V=V1;
...
figure
plot(Fourth_O_eig_matrix(:,1),'o','linewidth',1)
hold on
plot(Fourth_O_eig_matrix(:,2),'g*','linewidth',1)
plot(Fourth_O_eig_matrix(:,3),'bD','linewidth',1)
legend('V=35 m/s', "V=60m/s","V=85m/s")
axis equal
grid minor
title('Plotting all Eigen values locus for variation in speeds')
xlabel( '{\zeta\omega_n}')
ylabel('{\omega_d}')

figure
plot(EigShort_all(:,1),'o','linewidth',1)
hold on
plot(EigShort_all(:,2),'g*','linewidth',1)
plot(EigShort_all(:,3),'bD','linewidth',1)
legend('V=35 m/s', "V=60m/s","V=85m/s")
% axis square
grid minor
...
figure
plot(EigLong_all(:,1),'o','linewidth',1)
hold on
plot(EigLong_all(:,2),'g*','linewidth',1)
plot(EigLong_all(:,3),'bD','linewidth',1)
legend('V=35 m/s', "V=60m/s","V=85m/s")
% axis square
grid minor
...
Mderiv=[Mu;Mw;Mw_dot;Mq]
Zderiv=[Zu;Zw;Zw_dot;Zq]
%% PLotting Zeta vs omega for phugoid mode
[zeta' ome'];
figure
plot(zeta',ome')
xlabel({'\zeta'})
...
for j=1:length(V)
    plot(EigLong_all(:,j),'o','linewidth',1)
    hold on
end
grid minor
xlabel( '{\zeta\omega_n}')
ylabel('{\omega_d}')
title('Variation in speed for phugoid mode')
legend('25m/s','26m/s')

figure

for j=1:length(V)
    plot(EigShort_all(:,j),'o','linewidth',1)
    hold on
end
toc
