...
global CZ_deltaE
global A4long B4long theta0  ef dt Kp Ki Kd W S
global  thetaold
...
if ef==1
    ...
    xlabel('time (s)')
    ylabel('theta (rad)')
    legend('theta PID controller','theta w/o controller')
    subtitle('Effect of PID on Theta')
...
elseif ef==2
    ...
    xlabel('time (s)')
    ylabel('theta (rad)')
    legend('theta PD controller','theta w/o controller')
    subtitle('Effect of PD on Theta')
...
elseif ef==3
    ...
    xlabel('time (s)')
    ylabel('theta (rad)')
    legend('theta PI controller','theta w/o controller')
    subtitle('Effect of PI on Theta')
...
function [dyndt]=PID_theD(t,yn) %deltaE_t
global A4long dt thetaold B4long  Kp Ki Kd W S  CLalp CL_deltaE Cmalp Cm_deltaE CL0 Cm0
dyndt=zeros(4,1);
u= yn(1); w=yn(2) ; q=yn(3); theta=yn(4);
% if  t>=1 && t<=dt
%     theta=theta+0.087266;
